...
/*    ORGN: MIT                                             */
/*    FILE: BarcoSimu2Kalman.cpp                                        */
/*    DATE:                                                 */
/************************************************************/

#include <iterator>
#include "MBUtils.h"
#include "BarcoSimu2Kalman.h"
...
BarcoSimu2Kalman::BarcoSimu2Kalman()
{
  m_iterations = 0;
  m_timewarp   = 1;
...
bool BarcoSimu2Kalman::OnNewMail(MOOSMSG_LIST &NewMail)
{
...
bool BarcoSimu2Kalman::OnConnectToServer()
{	
   RegisterVariables();
   return(true);
}
...
bool BarcoSimu2Kalman::Iterate()
{
    ...
    if(m_iterations!=0)
    {
		...
		//So notifica depois de 4 atualizacaoes
		...
		{	
			...
			//Valores medidos
		  	m_Comms.Notify("NAV_X", gpsx);//m
		  	m_Comms.Notify("NAV_Y",gpsy);//m
			m_Comms.Notify("NAV_HEADING",heading);//degree
			m_Comms.Notify("NAV_SPEED",wz);//rad/s
			ic=0;
		}
...
bool BarcoSimu2Kalman::OnStartUp()
{
  cout<<"Startup"<<endl;
  list<string> sParams;
...
void BarcoSimu2Kalman::RegisterVariables()
{
...
void BarcoSimu2Kalman::integrar_passo()
{	
